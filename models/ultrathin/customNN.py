    ...
    def __init__(self, multiplier=1.0, classes=1000, **kwargs):
        ...
        with self.name_scope():
            ...
            with self.features.name_scope():
                _add_conv(self.features, channels=int(22), kernel=3, pad=1, stride=2)
                _add_conv_dw(self.features, dw_channels=22, channels=22, stride=1)
                _add_conv_dw(self.features, dw_channels=22, channels=44, stride=2)
                _add_conv_dw(self.features, dw_channels=44, channels=88, stride=1)
                _add_conv_dw(self.features, dw_channels=88, channels=176, stride=2)
                _add_conv_dw(self.features, dw_channels=176, channels=176, stride=1)
                _add_conv_dw(self.features, dw_channels=176, channels=353, stride=2)
                _add_conv_dw(self.features, dw_channels=353, channels=502, stride=2)
                _add_conv_dw(self.features, dw_channels=502, channels=502, stride=1)
